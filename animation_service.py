...
【⚠️ 节点坐标硬约束（必须严格遵守）】
1. **所有节点 x/y 必须严格落在 diagram_region 范围内**（即 dr.x1 ≤ x ≤ dr.x2 且 dr.y1 ≤ y ≤ dr.y2），
   绝对不允许把节点放在正文段落、公式、参考文献、caption 文字、表格、页眉页脚上
2. **同一 label 不要重复出现**（如有两个 "Encoder"，只标第一个；多个相同模块用 label_zh 区分如"编码器1"）
3. **任意两节点 x/y 距离至少 0.05**（避免热区圆点重叠不可点）
4. 节点数 5-15 个为佳；超过 20 个说明在标装饰性元素，请精简到核心节点

【复合图与子区识别（重要）】
- 若图中包含明显的子图（如 (a)(b)(c) 子标签、Type 1/2/3 分类、独立的方框组、Legend 区域），
  **每个子图区域必须至少包含 1 个代表节点**（位置放在子图标题或子图中心），
  label 用子图标题原文，label_zh 用一句话概括该子图说明的内容
- 不要只识别最显著的主流程而忽略下方/旁边的子图——节点要覆盖图中**所有有意义的语义区块**
- 子图内若有多个同类项（如"7 种类型"列表），合并为 1 个代表节点，label_zh 用"X类的Y"形式概括
...
- 至少返回 2 个节点
- 若图中含 (a)(b)(c) 子图或独立子区域，每个子区域至少 1 个代表节点，覆盖所有有意义的图形区块

【⚠️ 节点坐标硬约束（必须严格遵守）】
1. 所有节点 x/y **必须严格落在 diagram_region 范围内**，不允许放在正文/公式/caption 文字上
2. 同一 label 不重复（多个相同模块用 label_zh 区分）
3. 任意两节点 x/y 距离至少 0.05
...
def _enforce_node_constraints(nodes: list, dr: dict) -> list:
    """
    强制执行节点硬约束（即使 Qwen 不听话也兜底）：
    1. 越界节点（落在 diagram_region 外）直接丢弃
    2. 重复 label 只保留第一个
    3. 移除明显异常坐标（NaN、超 0/1 范围）

    返回过滤后的 nodes 列表 + 同时打印过滤统计（debug 用）
    """
    if not nodes:
        return nodes

    x1 = max(0.0, float(dr.get("x1", 0.0)))
    y1 = max(0.0, float(dr.get("y1", 0.0)))
    x2 = min(1.0, float(dr.get("x2", 1.0)))
    y2 = min(1.0, float(dr.get("y2", 1.0)))
    # 给个 5% 容差，避免 Qwen 给的坐标刚好压在 dr 边缘被误杀
    pad = 0.05
    bx1, by1 = max(0.0, x1 - pad), max(0.0, y1 - pad)
    bx2, by2 = min(1.0, x2 + pad), min(1.0, y2 + pad)

    out = []
    seen_labels = set()
    dropped_oob = 0      # out of bounds
    dropped_dup = 0      # duplicate label
    dropped_nan = 0      # NaN/extreme
    ...
    for n in nodes:
        ...
        if not (bx1 <= x <= bx2 and by1 <= y <= by2):
            dropped_oob += 1
            continue
        # 用 label（原文）去重，label_zh 不可靠
        label_key = (n.get("label") or "").strip().lower()
    ...
    if dropped_oob or dropped_dup or dropped_nan:
        print(f"[node_filter] 过滤 {dropped_oob} 越界 + {dropped_dup} 重复 + "
              f"{dropped_nan} 异常 / 共 {len(nodes)} → 剩 {len(out)}")
    return out
...
def _build_overlay_html(graph_json: dict, image_bytes: bytes, knowledge: dict) -> str:
    ...
    # 硬约束兜底：丢掉越界节点 + 重复 label（不依赖 Qwen 自觉遵守 prompt）
    nodes = _enforce_node_constraints(nodes, dr)

    # Pass-2 坐标校准：在整页图坐标系下，让 Qwen 对照原图修正偏差
    nodes = _verify_node_positions(image_bytes, nodes)

    # 校准后可能又出现越界（Qwen pass-2 也可能乱给），再过滤一次
    nodes = _enforce_node_constraints(nodes, dr)
